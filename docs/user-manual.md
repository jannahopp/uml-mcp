...
2. **Verify** (optional):
   ...
   ```
   You should see the server name and the diagram tool `generate_uml`.
...
| Tool            | Purpose |
|-----------------|--------|
| `generate_uml`  | Single tool for all diagram types. Pass `diagram_type` (class, sequence, mermaid, d2, etc.), `code`, and optionally `output_dir`, `output_format`, `theme`. Use `uml://types` for valid types. |

Full reference: [API / tools](api/tools.md).
